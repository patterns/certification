...
batch_size = 8
temperature = 5
teacher_preds_soft = model.predict(X_train_norm)####, batch_size=8) / temperature
...
##combined_label = list(zip(y_train_ohenc , teacher_preds_soft))
weight_a = 0.5
def distillation_loss(y_true_combined, y_pred):
    # split y_true_combined into y_true_hard and y_true_soft
    y_true_hard = y_true_combined[:, :3]
    y_true_soft = y_true_combined[:, 3:]
    # student vs. true labels
    hard_loss = tf.keras.losses.CategoricalCrossentropy()(y_true_hard, y_pred)
    # student vs. teacher predictions
    soft_loss = tf.keras.losses.CategoricalCrossentropy()(
        tf.nn.softmax(y_true_soft / temperature),
...
# is validation_split a param to the optimizer?
for epoch in range(10):  # Number of epochs
    print(f"Epoch {epoch + 1}/10")
    # does batch_size=8 mean to divide the combined set into 8 sublists
    #for (x_batch, y_combined_batch) in zip(X_train_norm, combined_label):
    for i in range(0, len(X_train_norm), batch_size):
        start = i
        stop = i + batch_size
        y_combined_batch = np.concatenate((y_train_ohenc[start:stop] , teacher_preds_soft[start:stop]), axis=1)
        x_batch = X_train_norm[start:stop]
        with tf.GradientTape() as tape:
            # Forward pass
            y_pred = student_model(x_batch, training=True)
            # compute loss
            loss = distillation_loss(y_combined_batch, y_pred)
